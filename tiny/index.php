 ...
 * Combined Main Page + Redirect Logic
...
$requestPath = trim($_SERVER['PATH_INFO'] ?? $_SERVER['REQUEST_URI'], '/');

// Remove "tiny/" prefix if it appears
$id = preg_replace('#^tiny/#', '', $requestPath);
...
// Handle form submission (if not a redirect)
$newShort = null;
...
<head>
    ...
    <style>
        ...
        .glass-card{background: rgba(255,255,255,0.9);backdrop-filter:blur(10px);border:1px solid rgba(255,255,255,0.2);box-shadow:0 25px 50px -12px rgba(0,0,0,0.05);}
        .brand-gradient{background:linear-gradient(135deg,#2563eb 0%,#7c3aed 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;}
        .btn-primary{background:linear-gradient(135deg,#2563eb 0%,#1d4ed8 100%);transition:all 0.2s ease;}
        .btn-primary:hover{transform:translateY(-1px);box-shadow:0 10px 15px -3px rgba(37,99,235,0.3);}
    </style>
</head>
            ...
            <form method="POST" class="space-y-4">
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="url" name="long_url" id="long_url" class="block w-full pl-11 pr-4 py-4 bg-gray-50 border border-gray-200 rounded-2xl focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all text-gray-900 placeholder-gray-400" placeholder="https://example.com/very/long/path/to/something" required>
                    </div>
                </div>
            ...
            <?php if ($newShort): ?>
                <div class="mt-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <div class="bg-blue-50 border border-blue-100 rounded-2xl p-6">
                        ...
                        <div class="flex items-center space-x-2">
                            <input type="text" id="shortResult" readonly value="<?= htmlspecialchars($newShort) ?>" class="bg-white border border-blue-200 text-blue-800 font-medium px-4 py-3 rounded-xl w-full focus:outline-none">
                            <button onclick="copyLink()" class="bg-blue-600 text-white p-3 rounded-xl hover:bg-blue-700 transition-colors" title="Copy to clipboard">
                                <i data-lucide="copy" id="copyIcon" class="w-6 h-6"></i>
                            </button>
                        </div>
                        <p class="mt-3 text-xs text-blue-500">This link will redirect users to your original destination.</p>
                    </div>
                </div>
            <?php endif; ?>
        </div>
